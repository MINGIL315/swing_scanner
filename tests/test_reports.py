...
class TestHtmlReport:
    ...
    def test_generate_creates_pattern_list_pages(self, tmp_path: Path) -> None:
        """4개 패턴 HTML 파일이 생성된다."""
        from scanner.reports.html_report import generate_daily_report, _PATTERN_KEYS
...
class TestExportToCsv:
    def test_csv_created(self, tmp_path: Path) -> None:
        from scanner.reports.excel_export import export_to_csv
    ...
    def test_csv_has_all_required_columns(self, tmp_path: Path) -> None:
        from scanner.reports.excel_export import export_to_csv, _COLUMNS
...
class TestExportToExcel:
    def test_excel_created(self, tmp_path: Path) -> None:
        from scanner.reports.excel_export import export_to_excel, _COLUMNS
