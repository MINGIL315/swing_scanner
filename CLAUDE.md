...
## 7. 거래량 필터 (Volume Momentum Filter)

| 시장 | 조건 |
| --- | --- |
| 한국 / 미국 | **최근 5일 거래량 평균 > 직전 20일 거래량 평균** |

> 옛 일평균 거래대금 컷(KR 50억원, US 5천만 USD)은 폐기되었다 — KOSPI200
> 우량주 단계에서 사실상 모든 종목이 통과해 의미가 작아 비활성. KOSPI 외
> 일반/KOSDAQ 도입 시 재활성화 검토 (2026-05-09 결정).
>
> 거래대금 데이터 자체는 ``OHLCVDaily.value`` 컬럼에 계속 적재된다.
...
> PER / 부채비율은 분석/필터에 사용하지 않는다.
> - PER: 적자/흑자 부호의 단순 컷이 스윙매매 차트 판단과 부합하지 않아 폐기 (2026-05-08).
> - 부채비율: KOSPI200 우량주만 다루는 현 단계에서는 의미가 작아 비활성. KOSPI200 외
>   일반 KOSPI / KOSDAQ 종목을 도입하는 시점에 다시 활성화 검토 (2026-05-09).
>
> 두 지표 모두 데이터 수집은 유지한다 (DB 의 ``Fundamental.per`` / ``debt_ratio`` 컬럼).
